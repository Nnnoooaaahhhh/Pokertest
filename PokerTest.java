	    ...
	    private static void gameStart() {
	       for (int i = 0; i < games; i++) {
	    		...
	    		for (int y = 0; y < numberHandCards; y++) {
	    			int rndm=r.nextInt(cardDeck.length-1);
	    			temp2 = cardDeck[cardDeck.length-y-1];
	    			temp1=cardDeck[rndm];
	    			cardDeck[rndm]=temp2;
	    			newCards[y]=temp1;
	    			cardDeck[cardDeck.length-y-1]=temp1;   		
	    		}     
	    		sorter();
	    		combs(); 
	        }
	   ...
	   static boolean onePair() {
	    ...
	    static boolean fullHouse() {	
	    	...
	    	int counter2 = 0;
	    		...
	    		if(counter1==1) paircount++;
	    		if(counter1==2) threecount++;
	    		
	    		if (paircount==1) {
	    			for (int y = 2; y < symbols.length-1; y++) {	
	        					if(symbols[2]==symbols[y+1]) {
	        						counter2++; 
	        						if(counter2==2) threecount++;
	        				}		
	        		}
	    		}
	    		if (threecount==1) {
	    ...
	    private static boolean row(int cards[], int desiredCount) {
	        int count = 0;
